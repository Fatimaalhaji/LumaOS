...
export async function upsertMemoryAction(formData: FormData) { const id = await userId(); const input = memorySchema.parse(Object.fromEntries(formData)); const memoryId = String(formData.get("id") || ""); if (memoryId) await db.update(memories).set({ ...input, updatedAt: new Date() }).where(and(eq(memories.id, memoryId), eq(memories.userId, id))); else await db.insert(memories).values({ userId: id, ...input }); revalidatePath("/memory"); }
export async function deleteMemoryAction(memoryId: string) { const id = await userId(); await db.delete(memories).where(and(eq(memories.id, memoryId), eq(memories.userId, id))); revalidatePath("/memory"); }
